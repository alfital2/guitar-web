// Threads — flowing line field (reads like vibrating strings). Ported from
// react-bits (MIT + Commons Clause); shader verbatim. Vendored ogl.
import { Renderer, Program, Mesh, Triangle, Color } from '../vendor/ogl.module.js';
...
export function createThreads(container, opts = {}) {
  const { accent = [0.94, 0.71, 0.27], fps = 30 } = opts;
  ...
  const stop = rafLoop(t => {
    const s = t * 0.001;
    // Autonomous drift (replaces cursor input): gentle sway of the line field.
    program.uniforms.uMouse.value[0] = 0.5 + 0.3 * Math.sin(s * 0.06);
    program.uniforms.uMouse.value[1] = 0.5 + 0.2 * Math.sin(s * 0.045);
    program.uniforms.iTime.value = s;
